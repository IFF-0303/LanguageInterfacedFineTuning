...
def main() -> None:
    ...
    def _normalise_inputs(text_batch: Any) -> List[str]:
        """Convert SHAP masker payloads into raw text strings for tokenisation."""
        ...
        for item in text_batch:
            ...
            # SHAP's text masker may provide lightweight wrapper objects containing
            # the token data in a ``data`` attribute or already tokenised as an
            # iterable of strings. We attempt to recover the original text in a
            # best-effort fashion before falling back to ``str(item)``.
            if hasattr(item, "data"):
                ...
                if isinstance(data, (list, tuple)):
                    normalised.append("".join(data))
                    continue

            if isinstance(item, (list, tuple)):
                normalised.append("".join(map(str, item)))
                continue
    ...
    def predict_fn(texts: Any) -> np.ndarray:
        processed_texts = _normalise_inputs(texts)
