#!/usr/bin/env python
"""Train a BERT-based classifier that fuses text and structured data features."""
...
from src.lift.models.bert.bert_structured_fusion_classifier import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fine-tune a BERT encoder alongside structured feature embeddings "
    ...
    parser.add_argument("--model-name", default="bert-base-uncased", help="Backbone BERT identifier.")
    parser.add_argument("--train-backbone", action="store_true", help="Unfreeze the BERT backbone for joint training.")
    ...
    parser.add_argument(
        ...
        type=float,
        help="Optional learning rate for backbone parameters when --train-backbone is enabled.",
    )
...
def build_datasets(
    *,
    model: BertStructuredFusionClassifier,
...
def train(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    model.tokenizer.save_pretrained(backbone_dir)
