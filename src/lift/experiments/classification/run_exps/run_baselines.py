import argparse
import numpy as np
from lift.experiments.classification.utils.corrupt_labels import corrupt_labels
...
from lift.experiments.classification.utils.prepare_data import prepare_data
from lift.experiments.classification.models.baselines import clf_model, param_grids
...
EVAL_PREFIX = RESULTS_DIR / "evals" / "rnd_rerun_all_acc_clf_baselines"
CSV_PREFIX = RESULTS_DIR / "csv" / "rnd_rerun_all_acc_clf_baselines"
EVAL_PREFIX.parent.mkdir(parents=True, exist_ok=True)
CSV_PREFIX.parent.mkdir(parents=True, exist_ok=True)
EVAL_PREFIX_STR = str(EVAL_PREFIX)
CSV_PREFIX_STR = str(CSV_PREFIX)
...
def load_baseline_data(did, split):
    ...
    fname = DATA_DIR / f"{did}_dev_test_split.npy"
    if not os.path.isfile(fname):
        print('prepare data', did)
        prepare_data(did, context=False)
    ith_fname = DATA_DIR / f"{did}_train_val_test_split{split}.npy"
...
if args.task == cfgs.ACCURACY:
    log_fpath = f"{EVAL_PREFIX_STR}_{args.task}.txt"
    csv_fpath = f"{CSV_PREFIX_STR}_{args.task}.csv"
    ...
    f1 = []
    for data_id in data_ids:
        log_fpath = f'{EVAL_PREFIX_STR}_{args.task}_{data_id}.txt'
        csv_fpath = f'{CSV_PREFIX_STR}_{args.task}_{data_id}.csv'
        logf = open(log_fpath, 'a+')
        X_train, y_train, X_val, y_val, X_test, y_test = load_baseline_data(int(data_id), 0)
        X = np.concatenate([X_train, X_val], axis=0)
        y = np.concatenate([y_train, y_val], axis=0)
...
# Michael's changes
    ...
    for data_id in data_ids:
        for random_label_noise in random_label_noises:
            ...
            log_fpath = f'{EVAL_PREFIX_STR}_{args.task}_{data_id}_{random_label_noise}.txt'
            csv_fpath = f'{CSV_PREFIX_STR}_{args.task}_{data_id}_{random_label_noise}.csv'
            logf = open(log_fpath, 'a+')
