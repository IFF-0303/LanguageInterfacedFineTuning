...
from lift.experiments.classification.models.GPT3FineTuner import GPT3FineTuner
...
def load_data(did, index,mixup=False):
    if mixup:
        fname = DATA_DIR / f"{did}_dev_test_split_mixup.npy"
    else:
    ...
    if mixup:
        ith_fname = DATA_DIR / f"{did}_train_val_test_split{index}_mixup.npy"
    else:
        ith_fname = DATA_DIR / f"{did}_train_val_test_split{index}.npy"
...
def load_jsonl(did, idx, context=False,feature_name=False):
    ...
    if feature_name:
        for mode in ['train', 'val', 'test']:
            jsonl_files[mode] = str(
                DATA_DIR
                / f"{did}_split{idx}_{mode}_context_{context}_feature_names.jsonl"
            )
    else:
        for mode in ['train', 'val', 'test']:
            jsonl_files[mode] = str(
                DATA_DIR / f"{did}_split{idx}_{mode}_context_{context}.jsonl"
            )
    return jsonl_files
