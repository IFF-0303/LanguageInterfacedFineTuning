...
from lift.experiments.classification.utils.classification_data_generator import (
    array2prompts,
    df2jsonl,
)
from lift.experiments.classification.utils.classificationDataGen import (
    dataGenerate,
)
from lift.experiments.classification.utils.helper import log
import lift.experiments.classification.utils.configs as cfgs
from lift.experiments.classification.models.GPTJFineTuner import GPTJFineTuner
from lift.experiments.classification.models.baselines import clf_model_teachers
from lift.experiments.classification.run_exps.run_exps_helper import *
from lift.experiments.classification.utils.train_nn_generator import DataNN
from lift.experiments.classification.utils.helper import add_ending_symbol
from lift.experiments.classification.models import lora_gptj
from lift.experiments.classification.plotters.plotter import (
    plot_decision_boundry_custom_color,
)
...
for directory in (EVALS_DIR, CSV_DIR, PLOTS_DIR):
    directory.mkdir(parents=True, exist_ok=True)

TEACHER_DATA_DIR = DATA_DIR / "teacher_datasets"
TEACHER_DATA_DIR.mkdir(parents=True, exist_ok=True)
NNET_DIR = DATA_DIR / "nnet"
NNET_DIR.mkdir(parents=True, exist_ok=True)
...
def csv_output_path() -> Path:
    return CSV_DIR / (
        f"run_gptj_{args.task}_featname_{args.use_feature_name}_"
        f"incon_{args.in_context}_subset_{args.subset}_"
        f"fraction_{args.subset_fraction}_did_{args.data_id}_"
        f"runidx_{args.run_idx}.csv"
    )
if args.in_context:
...
log_fpath = (
    EVALS_DIR
    / f"run_gptj_{args.task}_featname_{args.use_feature_name}_incon_{args.in_context}_subset_{args.subset}_fraction_{args.subset_fraction}.txt"
)
...
if args.task == cfgs.ACCURACY:
    ...
    for data_id in data_ids:
        ...
        for run_idx in range(1):
            ...
            jsonl_train_sampled = TEACHER_DATA_DIR / f'dataset_{data_id}_teachermodel_train.jsonl'
            ...
            filename_rest = TEACHER_DATA_DIR / f'dataset_{data_id}_teachermodel_rest.csv'
            ...
            for model_name in clf_model_teachers:
                ...
                for clf_name,clf in clf_dict.items():
                    try:
                        filename=str(
                            TEACHER_DATA_DIR
                            / f'dataset_{data_id}_teachermodel_{model_name}_{clf_name}.csv'
                        )
    ...
    for epoch in epochs:
        ...
        model_path = NNET_DIR / f"model_at_ep_{epoch}.pth"
