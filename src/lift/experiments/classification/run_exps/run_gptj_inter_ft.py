...
from lift.experiments.classification.utils.helper import query, log
...
fname = DATA_DIR / f'test_clf_{did}_inter_tuning.npy'
if os.path.isfile(fname):
    ...
    y_test = data['y_test']
    train_js1 = str(DATA_DIR / f'task1_clf_{did}_train_False.jsonl')
    train_js2 = str(DATA_DIR / f'task2_clf_{did}_train_False.jsonl')
    val_js1 = str(DATA_DIR / f'task1_clf_{did}_val_False.jsonl')
    val_js2 = str(DATA_DIR / f'task2_clf_{did}_val_False.jsonl')
    val_js = str(DATA_DIR / f'task3_clf_{did}_val_False.jsonl')
    test_js = str(DATA_DIR / f'task3_clf_{did}_test_False.jsonl')
    ...
    if did < 10:
        ...
        np.save(DATA_DIR / 'test_inter_tuning', {'X_train': X_train, 'X_val': X_val, 'X_test': X_test, 'y_val': y_val, 'y_test': y_test, 'y_train': y_train})
    else:
        ith_fname = DATA_DIR / f'{did}_train_val_test_split0.npy'
