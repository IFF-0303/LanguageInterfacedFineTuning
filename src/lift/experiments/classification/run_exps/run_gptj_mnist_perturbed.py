...
from lift.experiments.classification.models import ddp_lora_gptj as lora_gptj
...
if args.noisy_train:
    train_js =  str(DATA_DIR / f'{permuted}{data_name}{noisy}{eps}_train.jsonl')
    val_js = str(DATA_DIR / f'{permuted}{data_name}{noisy}{eps}_val.jsonl')
else:
    train_js =  str(DATA_DIR / f'{permuted}{data_name}_train.jsonl')
    val_js = str(DATA_DIR / f'{permuted}{data_name}_val.jsonl')

if is_noisy:
    test_js = str(DATA_DIR / f'{permuted}{data_name}{noisy}{eps}_test.jsonl')
else:
    test_js = str(DATA_DIR / f'{permuted}{data_name}{adv}{source}{eps}{noisy}{sigma}_test.jsonl')
print(train_js)
print(val_js)
print(test_js)
...
if args.eval == 0:
    model_name = GPTJ_RESULTS_DIR / f'{fname}/pytorch_model.bin'
    print(model_name)
    device = torch.device("cuda") if torch.cuda.is_available() else 'cpu'
    ...
    gpt.save_networks(output_dir=str(GPTJ_RESULTS_DIR / f'{fname}/'))
else:
