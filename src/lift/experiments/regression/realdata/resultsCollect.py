...
def save_csv(file_list, file_path, metric = 'RAE', grid_loss = False):
    ...
    for file in file_list:
        if file.endswith("all.json"):
            ...
            try:
                train_df = pd.read_csv(DATA_DIR / dataname / f"{dataname}_train_{n_train}_num.csv")
            except: 
                train_df = pd.read_csv(DATA_DIR / dataname / f"{dataname}_train_{n_train}.csv")
            X_train = train_df[train_df.columns[:-1]].values
            y_train = train_df[train_df.columns[-1]].values
            
            
            try:
                test_df = pd.read_csv(DATA_DIR / dataname / f"{dataname}_test_num.csv")
            except:
                test_df = pd.read_csv(DATA_DIR / dataname / f"{dataname}_test.csv")
