    ...
    for data in data_list:
        for prefix in prefix_list:
            ...
            for pc in pc_list:
                ...
                train_df = pd.read_csv(data_dir / f"{data.lower()}_train_{pc}.csv")
                valid_df = pd.read_csv(data_dir / f"{data.lower()}_valid.csv")
                test_df = pd.read_csv(data_dir / f"{data.lower()}_test.csv")
                ...
                train_file = str(data_dir / f"{data_prefix}{pc}_train.jsonl")
                valid_file = str(data_dir / f"{data_prefix}valid.jsonl")

                test_prompts = []
                with open(data_dir / f"{data_prefix}test.jsonl", 'r') as fp:
                    for line in fp:
                        json_obj = json.loads(line)
                        test_prompts.append(json_obj['prompt'])

                valid_prompts = []
                with open(data_dir / f"{data_prefix}valid.jsonl", 'r') as fp:
                    for line in fp:
                        json_obj = json.loads(line)
