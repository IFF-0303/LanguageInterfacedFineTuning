...
from lift.experiments.regression.utils.resultsCollector import regressionLoss
from lift.experiments.regression.utils.GPT3FineTuner import GPT3FineTuner
from lift.experiments.regression.utils.regressionDataGen import (
    dataGenerate,
    generate_data,
    load_openml_data,
)
...
try:
    from lift.experiments.regression.utils.GPTJFineTuner import GPTJFineTuner
...
def run_setting_gpt3(
    ...
    *,
    ...
    n_valid: int = 50,
    n_test: int = 100,
    data_list: Sequence[str | int] = ('linear',),
    p_list: Sequence[int] = (1, 50, 120),
    integer_list: Sequence[bool] = (False,),
    lb_ub_list: Sequence[tuple[float, float]] = ((-10, 10),),
    noise_level_list: Sequence[float] = (0.1,),
    beta: Sequence[float] | None = None,
    resolution: int = 200,
    num_epochs: int = 10,
    batch_size: int = 5,
    grid_int: Sequence[float] | None = None,
    donut: bool = False,
    outliers: tuple[np.ndarray, np.ndarray] | None = None,
    use_name_list: Sequence[bool] = (False,),
    lr_list: Sequence[float] = (0.05, 0.1, 0.2),
    openai_key='[replace it with your openai key]',
    valid_temperature: float = 0.75,
):
    data_dir = _ensure_dir(Path(data_dir) if data_dir is not None else paths.DATA_DIR)
    openai.api_key = openai_key
    for sim_idx in range(n_sims):
        data_sim_dir = _ensure_dir(data_dir / f'data_{sim_idx + 1}')
                    ...
                    for noise_level in noise_level_list_:
                        for use_name in use_name_list_:
                            ...
                            for sim_idx in range(n_sims):
                                ...
                                data_sim_dir = data_dir / f'data_{sim_idx + 1}'
                                # run exps
...
def run_setting_gptj(
    ...
    *,
    ...
    n_valid: int = 50,
    n_test: int = 100,
    data_list: Sequence[str] = ('linear',),
    p_list: Sequence[int] = (1, 50, 120),
    integer_list: Sequence[bool] = (False,),
    lb_ub_list: Sequence[tuple[float, float]] = ((-10, 10),),
    noise_level_list: Sequence[float] = (0.1,),
    epochs: Sequence[int] = (2, 6, 10),
    cuda_idx: int = 0,
    metric: str = 'RAE',
    batch_size: int = 4,
):
